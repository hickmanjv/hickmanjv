...
public class PlayerController implements Initializable {
    ...
    private void setupUI(String song){
        ...
        System.out.println("External Form: " + this.getClass().getResource("/music/" + song).toExternalForm());
        
        // like file i/o
        media = new Media(this.getClass().getResource("/music/" + song).toExternalForm());
        ...
        mediaPlayer.setAudioSpectrumListener(
                (double timestamp, double duration, float[] magnitudes, float[] phases) -> {
                    ...
                    if(magnitudes.length >= 4 && phases.length >= 4){
                        ...
                        // up-down
                        ellipse1.setCenterY( (Math.abs(magnitudes[0]) * 5) - 300);      // normalizing the value after increasing movement 5*60 = 300 so subtract 300
                        ...
                        ellipse1.setFill(Color.hsb(magnitudes[0] * -6.0, 1.0, 1.0));
                        ellipse2.setFill(Color.hsb(magnitudes[1] * -6.0, 1.0, 1.0));
                        ellipse3.setFill(Color.hsb(magnitudes[2] * -6.0, 1.0, 1.0));
                        ellipse4.setFill(Color.hsb(magnitudes[3] * -6.0, 1.0, 1.0));
